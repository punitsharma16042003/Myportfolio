...
document.addEventListener('DOMContentLoaded', () => {
  ...
  renderGallery(PORTFOLIO_DATA.gallery);
...
/**
 * 10. Render Research Memories Gallery (Dynamic GitHub API + Fallback)
 */
window.allGalleryItems = [];
window.activeGalleryFilter = 'all';

async function renderGallery(fallbackItems = []) {
  const container = document.getElementById('gallery-grid');
  if (!container) return;

  // Initialize with fallback or pre-configured items
  let items = Array.isArray(fallbackItems) ? [...fallbackItems] : [];

  // Show immediate render with local items
  window.allGalleryItems = items;
  displayGalleryCards(window.allGalleryItems, window.activeGalleryFilter);

  // Attempt automatic discovery from GitHub API
  const repoOwner = 'punitsharma16042003';
  const repoName = 'Myportfolio';
  const apiUrl = `https://api.github.com/repos/${repoOwner}/${repoName}/contents/gallery`;
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (Array.isArray(files)) {
        const imageExtensions = ['.jpg', '.jpeg', '.png', '.webp', '.svg', '.gif', '.bmp'];
        ...
        files.forEach(file => {
          if (file.type === 'file') {
            ...
            if (imageExtensions.includes(ext)) {
              // Does this item already exist in items?
              const exists = items.some(it => 
                it.image === file.path || 
                it.image.endsWith('/' + fileName) ||
                it.image === fileName
              );
              ...
              if (!exists) {
                ...
                const cleanTitle = fileName
                  ...
                  .replace(/^[\d_-]+/, '') // remove leading dates/numbers
                ...
                items.unshift({
                  ...
                  image: file.path,
                  title: cleanTitle,
                  caption: `Research memory photo uploaded directly to GitHub (${fileName}).`,
                  category: "Research Memory",
                  date: "Lab Memory",
                  location: "IIT Ropar / Research"
                });
                newItemsFound = true;
              }
            }
        ...
        if (newItemsFound || items.length > 0) {
          window.allGalleryItems = items;
          displayGalleryCards(window.allGalleryItems, window.activeGalleryFilter);
        }
      }
    ...
    console.info('Using local gallery items (GitHub API offline or rate-limited):', e.message);
  }
}

function displayGalleryCards(items, filter = 'all') {
  const container = document.getElementById('gallery-grid');
  if (!container) return;
  ...
  const filtered = (filter === 'all')
    ...
    : items.filter(item => {
        ...
        return item.category.toLowerCase().includes(filter.toLowerCase()) || filter.toLowerCase().includes(item.category.toLowerCase());
      });

  window.filteredGalleryItems = filtered;

  if (filtered.length === 0) {
    container.innerHTML = `
      <div class="empty-state" style="grid-column: 1 / -1; text-align: center; padding: 3.5rem 1rem; background: var(--bg-card); border-radius: var(--radius-lg); border: 1px dashed var(--bg-card-border);">
        <div style="font-size: 2.5rem; margin-bottom: 0.5rem;">📷</div>
        <h3 style="font-size: 1.25rem; margin-bottom: 0.5rem; color: var(--text-primary);">No photos found in this category</h3>
        <p style="color: var(--text-muted); font-size: 0.95rem;">Upload new photos to the <code>gallery/</code> folder on GitHub to add memories!</p>
      </div>
    `;
    return;
  }
  ...
  container.innerHTML = filtered.map((item, index) => `
    <div class="gallery-card" data-gallery-index="${index}">
      <div class="gallery-img-container">
        <img src="${escapeHtml(item.image)}" alt="${escapeHtml(item.title)}" loading="lazy" class="gallery-img" onerror="this.onerror=null; this.src='profile.jpg';">
        <div class="gallery-overlay">
          <span class="gallery-zoom-icon">🔍</span>
          <span class="gallery-overlay-hint">Enlarge Photo</span>
        </div>
        <span class="gallery-tag">${escapeHtml(item.category || 'Research')}</span>
      </div>
      <div class="gallery-card-body">
        <div class="gallery-card-header">
          <h3 class="gallery-card-title">${escapeHtml(item.title)}</h3>
        </div>
        <p class="gallery-card-caption">${escapeHtml(item.caption || '')}</p>
        <div class="gallery-card-meta">
          ${item.date ? `<span class="gallery-meta-item">📅 ${escapeHtml(item.date)}</span>` : ''}
          ${item.location ? `<span class="gallery-meta-item">📍 ${escapeHtml(item.location)}</span>` : ''}
        </div>
      </div>
    </div>
  `).join('');
